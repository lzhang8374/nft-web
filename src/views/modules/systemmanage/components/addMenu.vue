<template>
  <div>
    <div class="addMenu-form-first">
      <div style="margin-bottom: 20px;">
        <el-radio-group v-model="form.type" @change="handleRadioSelectChange">
          <el-radio size="medium" :label="0">菜单</el-radio>
          <el-radio size="medium" :label="1">功能</el-radio>
        </el-radio-group>
      </div>
      <!-- 菜单 -->
      	...
      	this.$refs[this.form.type==0?'menuForms':'functionForms'].validate((valid) => {
      		if (valid) {
      			this.$confirm('确认提交?', '提示', {
      				...
      				this.$http.post('/api/sysmenu/addMenu', this.form).then(res => {
      					if (res.data.code != 0) {
