<template>
	<div>
		<div class="productUnitEdit-form-first">
			<el-form ref='forms' :model='form' :rules='rules' label-width='90px'>
				...
				<el-form-item label="组织类型" prop="unitType">
					<el-select v-model="form.unitType" placeholder="请选择组织类型" style="width: 100%;"
						@change="getparentUnit($event)" disabled>
				...
				<el-form-item label="父级组织" prop="parentUnitId">
					<el-cascader :key="cascaderKey" v-model="value" show-all-levels :options="options"
		...
		<div class='productUnitEdit-footer'>
...
<script>
	export default {
		...
		methods: {
			getData(){
			  this.value = this.dataForm.parentUnitId
			  console.log(this.value)
			  this.getparentUnit(this.dataForm.unitType)
			},
			handleChange(value) {
				this.form.parentUnitId = value[value.length - 1]
			},
			//处理部门树形数据
			getTreeData(data) {
				...
				this.$refs.forms.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交?', '提示', {
							...
							this.$http.post('/unit/factoryunit/updateFactoryUnit', this.form).then(res => {
								if (res.data.code != 0) {
