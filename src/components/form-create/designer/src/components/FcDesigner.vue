                    ...
                    <ElContainer style="height: 100%;">
                        ...
                        <ElMain v-show="activeTab==='form'">
                            <FormCreate :rule="form.rule" :option="form.option"
                                        :value.sync="form.value.form" @change="tableFormChange"></FormCreate>
                        </ElMain>
...
export default {
    ...
    props: ['menu', 'height', 'config','fields','tables','type'],
    computed: {
        height_() {
            const h = this.height;
            if (!h) return '100%';
    ...
    data() {
        ...
        return {
            ...
            form: {
                rule: form(this.tables,this.type),
                option: {
                    form: {
                        labelPosition: 'top',
                        size: 'mini'
    ...
    watch: {
        ...
        'type':function(newVal){
          this.form.rule.forEach((current,currentIndex)=>{
            if(current.field == 'formInDatabase'){
              current.props.disabled = newVal=='add'?false:true
              return
            }
          })
        },
        'fields': {
          handler(newVal){
            this.baseForm.api.updateRule('field',{options: [...newVal]})
            this.clearDragRule()
          },
          deep:true
        },
    ...
    methods: {
        ...
        tableFormChange(field, value, _, fapi){
          if(field == 'formInDatabase'){
        ...
        propChange(field, value, _, fapi) {
            if (this.activeRule && fapi[this.activeRule._id] === this.activeRule) {
                if (field.indexOf('formCreate') === 0) {
                    ...
                    field = lower(field);
                    if (field.indexOf('effect') === 0 && field.indexOf('>') > -1) {
                      // let realField = field.split('>')[1]
                      // if(realField == "dataDict"){
                      //   realField = "fetch"
                      // }
        ...
        init(){
          // let initFormRule = form(this.tables,this.type)
          // this.form.rule = JSON.parse(JSON.stringify(initFormRule))

          // let initFieldRule = field(this.fields)
          // this.baseForm.rule = JSON.parse(JSON.stringify(initFormRule))

          // let initValidateRule = validate()
          // this.validateForm.rule = JSON.parse(JSON.stringify(initValidateRule))
